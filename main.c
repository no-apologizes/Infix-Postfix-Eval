...
Token* lexical_analysis(const char *c) {
    ...
    if (!output) { fprintf(stderr, "Failed to allocate memory for output tokens"); exit(EXIT_FAILURE); }
...
int main(const int argc, char **argv) {
    ...
    resolve_arity(t, total_tokens);
    printf("===== Arity Resolvement =====\n\n");
    print_tokens(t, -1);
    printf("===== End of Arity Resolvement =====\n\n");
    int64_t out_count;
    Token *postfix = shunting_yard(t, total_tokens, &out_count);
